...
void SoftmaxWithFocalLossLayer<Dtype>::Forward_cpu(
  ...
  for (int i = 0; i < outer_num_; ++i) {
    for (int j = 0; j < inner_num_; j++) {
      ...
      Dtype softmax_output = std::max(prob_data[i * dim + label_value * inner_num_ + j],
                           Dtype(FLT_MIN));

      loss -= std::pow(1.0-softmax_output, gamma_) * log(softmax_output);
    ...
    for (int i = 0; i < outer_num_; ++i) {
      for (int j = 0; j < inner_num_; ++j) {
        ...
        if (has_ignore_label_ && label_value == ignore_label_) {
          ...
          Dtype x = prob_data[i * dim + label_value * inner_num_ + j];
          for (int c = 0; c < bottom[0]->shape(softmax_axis_); ++c) {
            Dtype oi = prob_data[i * dim + c * inner_num_ + j];
            Dtype softmax_derivative = -x*oi;
            if(c == label_value){
              softmax_derivative = x*(1-oi);
            }
            bottom_diff[i * dim + c * inner_num_ + j] = (std::min(std::pow(1-x,gamma_-1),Dtype(FLT_MAX))*(gamma_*x*std::log(std::max(x,Dtype(FLT_MIN)))+x-1)/x)*softmax_derivative;
          }
